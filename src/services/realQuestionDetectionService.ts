...
class QuestionDetectionService {
  ...
  private loadConfig(): void {
    ...
    if (config.azure.openai.apiKey && config.azure.openai.endpoint) {
      ...
      this.endpoint = localStorage.getItem('openai_endpoint') || 'https://api.openai.com/v1';
    }
  }
   ...
   */
  public async generateAnswer(question: string, context: string = ''): Promise<AIResponse> {
    if (!this.apiKey) {
    ...
    try {
      ...
      const isAzureEndpoint = this.endpoint?.includes('cognitiveservices.azure.com');
      ...
      const response = await fetch(this.endpoint!, {
        ...
        body: JSON.stringify({
          messages: [
            {
              role: 'system',
              content: 'You are a helpful AI assistant that provides accurate, concise answers to questions. Base your responses on the provided context when available.'
            },
            {
              role: 'user',
              content: prompt
  ...
  private buildPrompt(question: string, context: string): string {
    ...
    if (context.trim()) {
      ...
      prompt += 'Please answer the question based on the provided context. If the context doesn\'t contain enough information, provide a general answer but mention that more context would be helpful.';
    } else {
      prompt += 'Please provide a helpful and accurate answer to this question.';
    }
  ...
  private getFallbackResponse(question: string): AIResponse {
    ...
    const fallbacks: { [key: string]: string } = {
      ...
      'where': 'This is asking about location. Additional context would help provide a specific answer.',
    ...
    const fallbackAnswer = fallbacks[questionWord] || 'I would need more context to provide a specific answer to this question.';
  ...
  public async processTranscript(
    ...
    for (const question of questions) {
      ...
      if (this.apiKey) {
        try {
          const answer = await this.generateAnswer(question.text, question.context);
