...
export const exportSession = (
  ...
  const { format, includeTimestamps, includeQuestions, includeAIResponses, includeMetadata } = options;
  
  // Filter transcript based on options
  let filteredTranscript = session.transcript;
  
  if (!includeQuestions) {
    filteredTranscript = filteredTranscript.filter(item => item.type !== 'question');
  }
  
  if (!includeAIResponses) {
    filteredTranscript = filteredTranscript.filter(item => item.type !== 'ai_response');
  }

  const timestamp = new Date().toISOString().split('T')[0];
  const baseFilename = `${session.title.replace(/[^a-zA-Z0-9]/g, '_')}_${timestamp}`;

  switch (format) {
    case 'txt':
      return {
        content: exportToTxt(session, filteredTranscript, includeTimestamps, includeMetadata),
    ...
    case 'csv':
      return {
        content: exportToCsv(session, filteredTranscript, includeTimestamps, includeMetadata),
    ...
    case 'docx':
      // For now, return formatted text - would need a library like docx for true DOCX
      return {
        content: exportToTxt(session, filteredTranscript, includeTimestamps, includeMetadata),
    ...
    case 'pdf':
      // For now, return formatted text - would need a library like jsPDF for true PDF
      return {
        content: exportToTxt(session, filteredTranscript, includeTimestamps, includeMetadata),
...
const addSecondsToTime = (time: string, seconds: number): string => {
  ...
  return `${String(newHours).padStart(2, '0')}:${String(newMinutes).padStart(2, '0')}:${String(newSecs).padStart(2, '0')},000`;
};
...
export const shareViaEmail = (
  ...
  const body = encodeURIComponent(
    ...
    `Content:\n${exportedContent.substring(0, 1000)}${exportedContent.length > 1000 ? '...' : ''}`
  );
