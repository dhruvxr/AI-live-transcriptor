...
export const detectQuestionWithConfidence = (
  text: string
): { isQuestion: boolean; confidence: number; type: string } => {
  const normalizedText = text.toLowerCase().trim();
  let confidence = 0;
  let type = "unknown";
...
export const generateQuestionResponse = async (
  ...
  try {
    ...
    return {
      response:
        "I apologize, but I'm unable to generate a response at the moment. Please try again later or rephrase your question.",
      confidence: 0.1,
      processingTime,
    };
  }
...
// Question analytics
export const analyzeQuestionPatterns = (
  questions: string[]
): {
  ...
  const averageLength =
    questions.reduce((sum, q) => sum + q.length, 0) / totalQuestions;
