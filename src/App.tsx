...
import { config, isAzureSpeechConfigured, isAzureOpenAIConfigured } from "./config/environment";
...
function EnhancedLiveTranscription({ onNavigate }: { onNavigate: any }) {
  ...
  const [questions, setQuestions] = useState<Array<{text: string, timestamp: string, confidence?: number, answer?: string}>>([]);
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      setIsRecording(false);
      alert('Failed to start recording. Please check your settings and microphone permissions.');
    }
  };
    ...
    <div style={{ 
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
          ...
          <h1 style={{ fontSize: '2rem', fontWeight: 'bold' }}>Live Transcription</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '2rem' }}>
            ...
            <div style={{ 
              ...
              <div style={{ marginBottom: '1rem' }}>
                ...
                <div style={{ fontSize: '0.875rem', marginBottom: '1rem' }}>
                  <div style={{ color: isAzureSpeechConfigured() ? '#10B981' : '#94A3B8', marginBottom: '0.25rem' }}>
                    {isAzureSpeechConfigured() ? '✅' : '⚠️'} Azure Speech Service: {isAzureSpeechConfigured() ? 'Configured' : 'Not configured'}
                  </div>
                  <div style={{ color: compatibility.azureSDK ? '#10B981' : '#94A3B8', marginBottom: '0.25rem' }}>
                    {compatibility.azureSDK ? '✅' : '⚠️'} Azure Speech SDK: {compatibility.azureSDK ? 'Loaded' : 'Loading...'}
                  </div>
                  <div style={{ color: compatibility.webSpeech ? '#10B981' : '#94A3B8', marginBottom: '0.25rem' }}>
                    {compatibility.webSpeech ? '✅' : '❌'} Web Speech API: {compatibility.webSpeech ? 'Supported' : 'Not supported'}
                  </div>
                  <div style={{ color: compatibility.canRequestMicrophone ? '#10B981' : '#F59E0B', marginBottom: '1rem' }}>
                    {compatibility.canRequestMicrophone ? '✅' : '⚠️'} Microphone Access: {compatibility.canRequestMicrophone ? 'Available' : 'Check permissions'}
                  </div>
                  
                  {/* Firefox-specific guidance */}
                  {compatibility.browser.isFirefox && !compatibility.azureSDK && (
                    <div style={{ 
                      ...
                      🦊 <strong>Firefox User:</strong> Loading Azure Speech SDK for full compatibility... 
                      If recording fails, try refreshing the page or use Demo Mode to see all features!
                    </div>
                  )}
                  ...
                  {compatibility.browser.isFirefox && compatibility.azureSDK && (
                    <div style={{ 
                      ...
                      🦊✅ <strong>Firefox + Azure SDK:</strong> Speech recognition ready! 
                      Click "🎤 Start Recording" to begin live transcription.
                    </div>
                  )}
                  
                  {!compatibility.overall && !compatibility.browser.isFirefox && (
                    <div style={{ 
                      ...
                      ⚠️ Speech recognition not available. Please use Chrome, Edge, or Safari, or try Demo Mode.
                    </div>
                  )}
                </div>
                  ...
                  {!isRecording ? (
                    <>
                      <button 
                        ...
                        style={{
                          backgroundColor: compatibility.canRequestMicrophone ? '#EF4444' : '#6B7280',
                          color: 'white',
                          padding: '1rem 2rem',
                          border: 'none',
                          borderRadius: '8px',
                          cursor: compatibility.canRequestMicrophone ? 'pointer' : 'not-allowed',
                          fontSize: '1rem',
                          fontWeight: 'bold',
                      ...
                      >
                        🎤 {compatibility.browser.isFirefox && !compatibility.azureSDK ? 'Request Microphone' : 'Start Recording'}
                      </button>
                      <button 
          ...
          <div>
            ...
            <div style={{ 
              ...
              <h3 style={{ fontSize: '1.25rem', marginBottom: '1rem' }}>Session Stats</h3>
              <div style={{ display: 'grid', gap: '1rem' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ color: '#94A3B8', fontSize: '0.875rem' }}>Words</div>
                </div>
              </div>
            </div>
                ...
                {questions.length > 0 ? (
                  questions.map((q, index) => (
                    <div key={index} style={{
                      ...
                      <div style={{ fontSize: '0.875rem', marginBottom: '0.5rem' }}>{q.text}</div>
                      {q.confidence && (
